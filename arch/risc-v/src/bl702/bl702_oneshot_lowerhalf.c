/****************************************************************************
 * arch/risc-v/src/bl702/bl702_oneshot_lowerhalf.c
 *
...
struct bl702_oneshot_lowerhalf_s
{
  /* This is the part of the lower half driver that is visible to the upper-
   * half client of the driver.  This must be the first thing in this
   * structure so that pointers to struct oneshot_lowerhalf_s are cast
   * compatible to struct bl702_oneshot_lowerhalf_s and vice versa.
   */
...
static int bl702_max_delay(struct oneshot_lowerhalf_s *lower,
                           struct timespec *           ts);
static int bl702_start(struct oneshot_lowerhalf_s *lower,
                       oneshot_callback_t              callback,
                       void *                      arg,
                       const struct timespec *     ts);
static int bl702_cancel(struct oneshot_lowerhalf_s *lower,
                        struct timespec *           ts);
...
/****************************************************************************
 * Name: bl702_oneshot_handler
 *
 * Description:
 *   Timer expiration handler
 *
 * Input Parameters:
 *   arg - Should be the same argument provided when bl702_oneshot_start()
 *         was called.
 *
 * Returned Value:
 *   None
...
static int bl702_oneshot_handler(int irq, void *context, void *arg)
{
  struct bl702_oneshot_lowerhalf_s *priv =
    (struct bl702_oneshot_lowerhalf_s *)arg;
  ...
  if (priv->tim == 0)
    {
      int_id = getreg32(BL702_TIMER_TMSR2);
      ticr_addr = BL702_TIMER_TICR2;
    }
  else
    {
      int_id = getreg32(BL702_TIMER_TMSR3);
      ticr_addr = BL702_TIMER_TICR3;
    }
...
{
  struct bl702_oneshot_lowerhalf_s *priv =
    (struct bl702_oneshot_lowerhalf_s *)lower;
  uint64_t usecs;
                                               ...
                                               uint16_t resolution)
{
  ...
  priv = (struct bl702_oneshot_lowerhalf_s *)kmm_zalloc(
    sizeof(struct bl702_oneshot_lowerhalf_s));
  ...
  if (priv->tim == TIMER_CH0)
    {
      priv->irq = BL702_IRQ_TIMER_CH0;
    }
  else
    {
      priv->irq = BL702_IRQ_TIMER_CH1;
    }

  /* Initialize the contained BL702 oneshot timer */
  ...
  bl702_timer_intmask(chan, TIMER_INT_ALL, 1);

  /* timer disable */

  bl702_timer_disable(chan);
