/****************************************************************************
 * arch/risc-v/src/bl702/bl702_netdev.c
 *
...
#include "bl702_os_hal.h"
#include "bl702_netdev.h"
#include "bl702_efuse.h"

#ifdef CONFIG_BL702_WIRELESS
...
/* BL702_NET_NINTERFACES determines the number of physical interfaces
 * that will be supported.
 */

#ifndef CONFIG_BL702_NET_MULTI_INTERFACE
#define BL702_NET_NINTERFACES 1
#else
#define BL702_NET_NINTERFACES 2
#endif

#define WIFI_MTU_SIZE 1516

#define BL702_NET_TXBUFF_NUM  12
#define BL702_NET_TXBUFF_SIZE (WIFI_MTU_SIZE + PRESERVE_80211_HEADER_LEN)

#define BL702_TXDESC_THRESHOLD 3

#if BL702_NET_TXBUFF_SIZE & 0x3 != 0
#error "BL702_NET_TXBUFF_SIZE must be aligned to 4 bytes"
#endif

#if !(BL702_TXDESC_THRESHOLD > 0)
#error "BL702_TXDESC_THRESHOLD invalid."
#endif

#define TX_BUFF_BIT_SIZE BL702_NET_TXBUFF_NUM
...
/* The bl702_net_driver_s encapsulates all state information for a single
 * hardware interface
 */

struct bl702_net_driver_s
{
...
{
  struct bl702_net_driver_s *priv;
...
typedef uint8_t (*tx_buff_t)[BL702_NET_TXBUFF_SIZE];
...
struct rx_pending_item_s
{
  ...
  struct bl702_net_driver_s *priv; /* Which interface should to deliver */
  uint8_t *        data;
  int              len;
};
...
struct bl702_net_driver_s g_bl702_net[BL702_NET_NINTERFACES];

static struct tx_buf_ind_s g_tx_buf_indicator =
  BITSET_T_INITIALIZER((1 << BL702_NET_TXBUFF_NUM) - 1);

static uint8_t locate_data(".wifi_ram.txbuff")
g_tx_buff[BL702_NET_TXBUFF_NUM][BL702_NET_TXBUFF_SIZE];
...
{
  .country_code = CONFIG_BL702_WIRELESS_CONTRY_CODE,
};
...
extern int bl702_ef_ctrl_read_mac_address(uint8_t mac[6]);
...
static int bl702_net_transmit(struct bl702_net_driver_s *priv);
static int bl702_net_txpoll(struct net_driver_s *dev);

/* Interrupt handling */

static void bl702_net_reply(struct bl702_net_driver_s *priv);
static void bl702_net_receive(struct bl702_net_driver_s *priv);

/* NuttX callback functions */

static int bl702_net_ifup(struct net_driver_s *dev);
static int bl702_net_ifdown(struct net_driver_s *dev);

static void bl702_net_txavail_work(void *arg);
static int  bl702_net_txavail(struct net_driver_s *dev);

#if defined(CONFIG_NET_MCASTGROUP) || defined(CONFIG_NET_ICMPv6)
static int bl702_net_addmac(struct net_driver_s *dev,
                            const uint8_t *mac);
# ifdef CONFIG_NET_MCASTGROUP
static int bl702_net_rmmac(struct net_driver_s *dev,
                           const uint8_t *mac);
# endif
# ifdef CONFIG_NET_ICMPv6
static void bl702_net_ipv6multicast(struct bl702_net_driver_s *priv);
# endif
#endif

#ifdef CONFIG_NETDEV_IOCTL
static int
bl702_net_ioctl(struct net_driver_s *dev, int cmd, unsigned long arg);
#endif
...
static int bl702_net_transmit(struct bl702_net_driver_s *priv)
{
  ...
  DEBUGASSERT(priv->push_cnt < BL702_TXDESC_THRESHOLD);

  if (!IFF_IS_RUNNING(priv->net_dev.d_flags))
    {
      nwarn("drop packet due to iface no carrier\n");
      bl702_netdev_free_txbuf(priv->net_dev.d_buf -
  ...
  if (priv->push_cnt == BL702_TXDESC_THRESHOLD)
    {
      /* notify to tx now */
...
{
  struct bl702_net_driver_s *priv =
    (struct bl702_net_driver_s *)dev->d_private;
  ...
  if (priv->net_dev.d_len > 0)
    {
      ...
      tx_p = bl702_netdev_alloc_txbuf();
      if (tx_p)
        {
          tx_p += PRESERVE_80211_HEADER_LEN;
          DEBUGASSERT(priv->net_dev.d_len <=
                 BL702_NET_TXBUFF_SIZE - PRESERVE_80211_HEADER_LEN);
  ...
  if (BUF->type == HTONS(ETHTYPE_ARP))
    {
      ...
      if (priv->net_dev.d_len > 0)
        {
          uint8_t *tx_p = bl702_netdev_alloc_txbuf();
          if (tx_p != NULL)
            {
              DEBUGASSERT(priv->net_dev.d_len <=
                     BL702_NET_TXBUFF_SIZE - PRESERVE_80211_HEADER_LEN);
...
static int bl702_launch_pending_rx(void)
{
...
/****************************************************************************
 * Name: bl702_net_notify
 *
 * Description:
 *   BL702 WiFi notify handler, similar interrupt function
 *
 * Input Parameters:
...
int bl702_net_notify(uint32_t event, uint8_t *data, int len, void *opaque)
{
  DEBUGASSERT(opaque != NULL);

  struct bl702_net_driver_s *priv =
    (struct bl702_net_driver_s *)opaque;

  DEBUGASSERT(priv == &g_bl702_net[0] || priv == &g_bl702_net[1]);

  int ret;

  if (event & BL702_NET_EVT_TX_DONE)
    {
      /* if we have tx buffer, we put pending input packet first */

      ret = bl702_launch_pending_rx();
      if (ret != OK)
        {
      ...
      if (work_available(&priv->availwork))
        {
          ...
          work_queue(
            ETHWORK, &priv->availwork, bl702_net_txavail_work, priv, 0);
        }
    }

  if (event & BL702_NET_EVT_RX)
    {
      int tx_buf_empty = 0;
...
static int bl702_net_ifup(struct net_driver_s *dev)
{
  ...
  bl702_net_ipv6multicast(priv);
#endif

  return OK;
}

static int bl702_net_soft_reset(void)
{
  int idx;
  ...
  for (idx = 0; idx < BL702_NET_NINTERFACES; idx++)
    {
      g_bl702_net[idx].current_mode = IW_MODE_AUTO;
    }
...
static int bl702_net_ifdown(struct net_driver_s *dev)
{
  ...
  if (priv == &g_bl702_net[0])
    {
      bl702_net_soft_reset();
    }
...
static void bl702_net_txavail_work(void *arg)
{
  struct bl702_net_driver_s *priv = (struct bl702_net_driver_s *)arg;
  ...
  if (priv->net_dev.d_buf)
    {
      devif_poll(&priv->net_dev, bl702_net_txpoll);
  ...
  if (work_available(&priv->availwork))
    {
      ...
      work_queue(ETHWORK, &priv->availwork, bl702_net_txavail_work, priv, 0);
    }
...
static void bl702_net_ipv6multicast(struct bl702_net_driver_s *priv)
{
  ...
  bl702_net_addmac(dev, g_ipv6_ethallnodes.ether_addr_octet);
  ...
  bl702_net_addmac(dev, g_ipv6_ethallrouters.ether_addr_octet);
...
static wifi_mgmr_t *
bl702_netdev_get_wifi_mgmr(struct bl702_net_driver_s *priv)
{
...
#ifdef CONFIG_NETDEV_IOCTL
static int bl702_ioctl_wifi_start(struct bl702_net_driver_s *priv,
                                  uintptr_t                      arg)
{
  UNUSED(arg);
  ...
  wifi_mgmr_t *mgmr = bl702_netdev_get_wifi_mgmr(priv);
  if (mgmr == NULL)
    {
      return -EPERM;
    }
...
static int bl702_ioctl_wifi_stop(struct bl702_net_driver_s *priv,
                                 uintptr_t                      arg)
{
  UNUSED(arg);
  ...
  switch (cmd)
    ...
    case SIOCSIWMODE: /* Set operation mode */
      do
        {
          struct iwreq *req = (struct iwreq *)arg;
#ifdef CONFIG_BL702_NET_MULTI_INTERFACE
          int interface_idx = priv - g_bl702_net;

          DEBUGASSERT(interface_idx >= 0 &&
              interface_idx < BL702_NET_NINTERFACES);
#endif
      ...
      do
        {
          ...
          if (req->u.essid.flags == 0)
            {
              return bl702_ioctl_wifi_stop(priv, arg);
            }
          else if (req->u.essid.flags == 1)
            {
              return bl702_ioctl_wifi_start(priv, arg);
            }
          else
            {
...
uint8_t *bl702_netdev_alloc_txbuf(void)
{
...
void bl702_netdev_free_txbuf(uint8_t *buf)
{
  irqstate_t irq;
  int        idx;
  ...
  DEBUGASSERT(idx < BL702_NET_TXBUFF_NUM && idx >= 0);
...
/****************************************************************************
 * Name: bl702_net_event
 *
 * Description:
 *   BL702 WiFi event handler, called by BL702 wifi manager private library
 *
 * Input Parameters:
 *   event: event number
...
void bl702_net_event(int evt, int val)
{
  net_lock();
  ...
  switch (evt)
    ...
    case CODE_WIFI_ON_CONNECTED:
      do
        {
          struct bl702_net_driver_s *priv = &g_bl702_net[0];
    ...
    case CODE_WIFI_ON_AP_STARTED:
      do
        {
#ifdef CONFIG_BL702_NET_MULTI_INTERFACE
          struct bl702_net_driver_s *priv = &g_bl702_net[1];
          netdev_carrier_on(&priv->net_dev);
#endif
          g_state.ap_stared = 1;
        }
...
int bl702_net_initialize(void)
{
  ...
  bl702_efuse_read_mac_address(mac);
  ...
  for (idx = 0; idx < BL702_NET_NINTERFACES; idx++)
    {
      ...
      priv = &g_bl702_net[idx];

      /* Initialize the driver structure */

      memset(priv, 0, sizeof(struct bl702_net_driver_s));
      priv->net_dev.d_ifup =
        bl702_net_ifup; /* I/F up (new IP address) callback */

      priv->net_dev.d_ifdown  = bl702_net_ifdown;  /* I/F down callback */
      priv->net_dev.d_txavail = bl702_net_txavail; /* New TX data callback */
#ifdef CONFIG_NET_MCASTGROUP
      priv->net_dev.d_addmac = bl702_net_addmac; /* Add multicast MAC address */
      priv->net_dev.d_rmmac  = bl702_net_rmmac;  /* Remove multicast MAC address */
#endif
#ifdef CONFIG_NETDEV_IOCTL
      priv->net_dev.d_ioctl = bl702_net_ioctl; /* Handle network IOCTL commands */
#endif
