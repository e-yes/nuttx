/****************************************************************************
 * arch/risc-v/src/bl702/bl702_tim.c
 *
...
/****************************************************************************
 * Name: bl702_data_setbits
 ****************************************************************************/

static void bl702_wdt_access(void)
{
  modifyreg32(BL702_TIMER_WFAR, TIMER_WFAR_MASK, 0xbaba);
  modifyreg32(BL702_TIMER_WSAR, TIMER_WSAR_MASK, 0xeb10);
}
...
uint32_t bl702_timer_getcompvalue(uint8_t timer_ch, uint8_t cmp_no)
{
  return getreg32(BL702_TIMER_TMR2_0 + \
                  4 * (TIMER_MAX_MATCH * timer_ch + cmp_no));
}

/****************************************************************************
 * Name: bl702_timer_setcompvalue
 *
 * Description:
...
void bl702_timer_setcompvalue(uint8_t timer_ch,
                              uint8_t cmp_no,
                              uint32_t val)
{
  putreg32(val,
           BL702_TIMER_TMR2_0 + 4 * (TIMER_MAX_MATCH * timer_ch + cmp_no));
}

/****************************************************************************
 * Name: bl702_timer_getcountervalue
 *
 * Description:
...
uint32_t bl702_timer_getcountervalue(uint32_t timer_ch)
{
  uint32_t count;
  uint32_t tcvwr_addr;
  ...
  tcvwr_addr = BL702_TIMER_TCVWR2 + 4 * timer_ch;
...
/****************************************************************************
 * Name: bl702_timer_getmatchstatus
 *
 * Description:
...
bool bl702_timer_getmatchstatus(uint32_t timer_ch, uint8_t cmp_no)
{
  uint32_t status = getreg32(BL702_TIMER_TMSR2 + 4 * timer_ch);
...
/****************************************************************************
 * Name: bl702_timer_getpreloadvalue
 *
 * Description:
...
uint32_t bl702_timer_getpreloadvalue(uint32_t timer_ch)
{
  return getreg32(BL702_TIMER_TPLVR2 + 4 * timer_ch);
}

/****************************************************************************
 * Name: bl702_timer_setpreloadvalue
 *
 * Description:
...
void bl702_timer_setpreloadvalue(uint8_t timer_ch, uint32_t val)
{
    putreg32(val, BL702_TIMER_TPLVR2 + 4 * timer_ch);
}

/****************************************************************************
 * Name: bl702_timer_setpreloadtrigsrc
 *
 * Description:
...
void bl702_timer_setpreloadtrigsrc(uint8_t timer_ch, uint32_t pl_src)
{
  putreg32(pl_src, BL702_TIMER_TPLCR2 + 4 * timer_ch);
}

/****************************************************************************
 * Name: bl702_timer_setcountmode
 *
 * Description:
...
void bl702_timer_setcountmode(uint32_t timer_ch, uint8_t count_mode)
{
  modifyreg32(BL702_TIMER_TCMR,
              TIMER_TCMR_TIMER2_MODE << timer_ch,
              count_mode ? TIMER_TCMR_TIMER2_MODE << timer_ch : 0);
}

/****************************************************************************
 * Name: bl702_timer_clearintstatus
 *
 * Description:
...
void bl702_timer_clearintstatus(uint8_t timer_ch, uint32_t cmp_no)
{
  putreg32(BL702_TIMER_TICR2 + 4 * timer_ch, 1 << cmp_no);
}

/****************************************************************************
 * Name: bl702_timer_init
 *
 * Description:
...
void bl702_timer_init(struct timer_cfg_s *timer_cfg)
{
  ...
  if (timer_ch == TIMER_CH0)
    {
      modifyreg32(BL702_TIMER_TCCR, TIMER_TCCR_CS_1_MASK,
                  timer_cfg->clk_src << TIMER_TCCR_CS_1_SHIFT);
    }
  else
    {
      modifyreg32(BL702_TIMER_TCCR, TIMER_TCCR_CS_2_MASK,
  ...
  if (timer_ch == TIMER_CH0)
    {
      modifyreg32(BL702_TIMER_TCDR, TIMER_TCDR_TCDR2_MASK,
                  timer_cfg->clock_division << TIMER_TCDR_TCDR2_SHIFT);
    }
  else
    {
      modifyreg32(BL702_TIMER_TCDR, TIMER_TCDR_TCDR3_MASK,
                  timer_cfg->clock_division << TIMER_TCDR_TCDR3_SHIFT);
    }

  /* Configure timer count mode: preload or free run */

  bl702_timer_setcountmode(timer_ch, timer_cfg->count_mode);

  /* Configure timer preload trigger src */

  bl702_timer_setpreloadtrigsrc(timer_ch, timer_cfg->pl_trig_src);

  if (timer_cfg->count_mode == TIMER_COUNT_PRELOAD)
    {
      /* Configure timer preload value */

      bl702_timer_setpreloadvalue(timer_ch, timer_cfg->pre_load_val);
    }

  /* Configure match compare values */

  bl702_timer_setcompvalue(timer_ch, 0, timer_cfg->match_val0);
  bl702_timer_setcompvalue(timer_ch, 1, timer_cfg->match_val1);
  bl702_timer_setcompvalue(timer_ch, 2, timer_cfg->match_val2);
}

/****************************************************************************
 * Name: bl702_timer_enable
 *
 * Description:
...
void bl702_timer_enable(uint8_t timer_ch)
{
  modifyreg32(BL702_TIMER_TCER, 0, (TIMER_TCER_TIMER2_EN << timer_ch));
}

/****************************************************************************
 * Name: bl702_timer_disable
 *
 * Description:
...
void bl702_timer_disable(uint8_t timer_ch)
{
  modifyreg32(BL702_TIMER_TCER, (TIMER_TCER_TIMER2_EN << timer_ch), 0);
}

/****************************************************************************
 * Name: bl702_timer_intmask
 *
 * Description:
...
void bl702_timer_intmask(uint8_t timer_ch, uint8_t int_type,
                         uint8_t int_mask)
{
  uint32_t tier_addr = BL702_TIMER_TIER2 + 4 * timer_ch;
...
void bl702_wdt_set_clock(uint8_t clk_src, uint8_t div)
{
  /* Configure watchdog timer clock source */

  modifyreg32(BL702_TIMER_TCCR, TIMER_TCCR_CS_WDT_MASK,
              clk_src << TIMER_TCCR_CS_WDT_SHIFT);

  /* Configure watchdog timer clock division */

  modifyreg32(BL702_TIMER_TCDR, TIMER_TCDR_WCDR_MASK,
              div << TIMER_TCDR_WCDR_SHIFT);
}

/****************************************************************************
 * Name: bl702_wdt_getmatchvalue
 *
 * Description:
...
uint16_t bl702_wdt_getmatchvalue(void)
{
  bl702_wdt_access();

  /* Get watchdog timer match register value */

  return (uint16_t)(getreg32(BL702_TIMER_WMR) & TIMER_WMR_WMR_MASK);
}

/****************************************************************************
 * Name: bl702_wdt_setcompvalue
 *
 * Description:
...
void bl702_wdt_setcompvalue(uint16_t val)
{
  bl702_wdt_access();

  /* Set watchdog timer match register value */

  putreg32(val, BL702_TIMER_WMR);
}

/****************************************************************************
 * Name: bl702_wdt_getcountervalue
 *
 * Description:
...
uint16_t bl702_wdt_getcountervalue(void)
{
  bl702_wdt_access();

  /* Get watchdog timer count register value */

  return (uint16_t)(getreg32(BL702_TIMER_WVR) & TIMER_WVR_WVR_MASK);
}

/****************************************************************************
 * Name: bl702_wdt_resetcountervalue
 *
 * Description:
...
void bl702_wdt_resetcountervalue(void)
{
  ...
  modifyreg32(BL702_TIMER_WCR, 0, TIMER_WCR_WCR);
}

/****************************************************************************
 * Name: bl702_wdt_getresetstatus
 *
 * Description:
...
bool bl702_wdt_getresetstatus(void)
{
  bl702_wdt_access();

  /* Get watchdog status register */

  return ((getreg32(BL702_TIMER_WSR) & TIMER_WSR_WTS) != 0);
}

/****************************************************************************
 * Name: bl702_wdt_clearresetstatus
 *
 * Description:
...
void bl702_wdt_clearresetstatus(void)
{
  bl702_wdt_access();

  /* Set watchdog status register */

  modifyreg32(BL702_TIMER_WSR, TIMER_WSR_WTS, 0);
}

/****************************************************************************
 * Name: bl702_wdt_enable
 *
 * Description:
...
void bl702_wdt_enable(void)
{
  bl702_wdt_access();

  modifyreg32(BL702_TIMER_WMER, 0, TIMER_WMER_WE);
}

/****************************************************************************
 * Name: bl702_wdt_disable
 *
 * Description:
...
void bl702_wdt_disable(void)
{
  bl702_wdt_access();

  modifyreg32(BL702_TIMER_WMER, TIMER_WMER_WE, 0);
}

/****************************************************************************
 * Name: bl702_wdt_intmask
 *
 * Description:
...
void bl702_wdt_intmask(uint8_t int_type, uint8_t int_mask)
{
  bl702_wdt_access();
      ...
      if (int_mask == 0)
        {
          ...
          modifyreg32(BL702_TIMER_WMER, TIMER_WMER_WRIE, 0);
        }
      else
        {
          /* Disable this interrupt */
          ...
          modifyreg32(BL702_TIMER_WMER, 0, TIMER_WMER_WRIE);
        }
