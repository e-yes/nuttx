/****************************************************************************
 * arch/risc-v/src/bl702/bl702_serial.c
 *
...
#define BL702_CONSOLE_TX     GPIO_UART0_TX
#define BL702_CONSOLE_RX     GPIO_UART0_RX
#define HAVE_UART
...
#define BL702_CONSOLE_TX     GPIO_UART1_TX
#define BL702_CONSOLE_RX     GPIO_UART1_RX
#define HAVE_UART
#endif
#endif /* HAVE_CONSOLE */
...
static int  bl702_setup(struct uart_dev_s *dev);
static void bl702_shutdown(struct uart_dev_s *dev);
static int  bl702_attach(struct uart_dev_s *dev);
static void bl702_detach(struct uart_dev_s *dev);
static int  bl702_ioctl(struct file *filep, int cmd, unsigned long arg);
static int  bl702_receive(struct uart_dev_s *dev, unsigned int *status);
static void bl702_rxint(struct uart_dev_s *dev, bool enable);
static bool bl702_rxavailable(struct uart_dev_s *dev);
static void bl702_send(struct uart_dev_s *dev, int ch);
static void bl702_txint(struct uart_dev_s *dev, bool enable);
static bool bl702_txready(struct uart_dev_s *dev);
static bool bl702_txempty(struct uart_dev_s *dev);
...
{
  .setup       = bl702_setup,
  .shutdown    = bl702_shutdown,
  .attach      = bl702_attach,
  .detach      = bl702_detach,
  .ioctl       = bl702_ioctl,
  .receive     = bl702_receive,
  .rxint       = bl702_rxint,
  .rxavailable = bl702_rxavailable,
#ifdef CONFIG_SERIAL_IFLOWCONTROL
  .rxflowcontrol = NULL,
#endif
  .send    = bl702_send,
  .txint   = bl702_txint,
  .txready = bl702_txready,
  .txempty = bl702_txempty,
};

/* I/O buffers */

#ifdef CONFIG_BL702_UART0
static char g_uart0rxbuffer[CONFIG_UART0_RXBUFSIZE];
static char g_uart0txbuffer[CONFIG_UART0_TXBUFSIZE];

static struct bl702_uart_s g_uart0priv =
{
  .irq      = BL702_IRQ_UART0,
  .config =
    {
...
#ifdef CONFIG_BL702_UART1
static char g_uart1rxbuffer[CONFIG_UART1_RXBUFSIZE];
static char g_uart1txbuffer[CONFIG_UART1_TXBUFSIZE];

static struct bl702_uart_s g_uart1priv =
{
  .irq      = BL702_IRQ_UART1,
  .config =
    {
...
static int __uart_interrupt(int irq, void *context, void *arg)
{
  ...
  struct bl702_uart_s *priv = dev->priv;
  uint8_t uart_idx          = priv->config.idx;
  uint32_t int_status;
  uint32_t int_mask;

  int_status = getreg32(BL702_UART_INT_STS(uart_idx));
  int_mask = getreg32(BL702_UART_INT_MASK(uart_idx));
...
static int bl702_setup(struct uart_dev_s *dev)
{
  struct bl702_uart_s *priv = (struct bl702_uart_s *)dev->priv;

  bl702_uart_configure(&priv->config);
  return OK;
}
...
static void bl702_shutdown(struct uart_dev_s *dev)
{
  ...
  modifyreg32(BL702_UART_UTX_CONFIG(uart_idx), UART_UTX_CONFIG_CR_EN, 0);
  modifyreg32(BL702_UART_URX_CONFIG(uart_idx), UART_URX_CONFIG_CR_EN, 0);
}
          ...
          if (ret == OK)
            {
              ...
              tmp_val = getreg32(BL702_UART_INT_MASK(config.idx));
              bl702_uart_configure(&config);
              putreg32(tmp_val, BL702_UART_INT_MASK(config.idx));
            }
        }
      while (0);
      break;
...
static int bl702_receive(struct uart_dev_s *dev, unsigned int *status)
{
  ...
  if (getreg32(BL702_UART_FIFO_CONFIG_1(uart_idx)) & \
      UART_FIFO_CONFIG_1_RX_CNT_MASK)
    {
      rxdata = getreg32(BL702_UART_FIFO_RDATA(uart_idx)) & \
  ...
  if (enable)
    {
      ...
      int_mask &= ~(UART_INT_MASK_CR_URX_FIFO_MASK);
      int_mask &= ~(UART_INT_MASK_CR_URX_END_MASK);
      putreg32(int_mask, BL702_UART_INT_MASK(uart_idx));
#endif
    }
  else
    {
...
static bool bl702_rxavailable(struct uart_dev_s *dev)
{
  ...
  return (getreg32(BL702_UART_FIFO_CONFIG_1(uart_idx)) & \
...
static void bl702_send(struct uart_dev_s *dev, int ch)
{
  ...
  while ((getreg32(BL702_UART_FIFO_CONFIG_1(uart_idx)) & \
         UART_FIFO_CONFIG_1_TX_CNT_MASK) == 0);

  putreg32(ch, BL702_UART_FIFO_WDATA(uart_idx));
}
