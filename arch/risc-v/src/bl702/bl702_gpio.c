/****************************************************************************
 * arch/risc-v/src/bl702/bl702_gpio.c
 *
...
int bl702_configgpio(gpio_pinset_t cfgset)
{
  ...
  if (!(cfgset & GPIO_INPUT))
    {
      modifyreg32(BL702_GPIO_CFGCTL34, 0, (1 << pin));
    }
...
{
  bl702_configgpio(GPIO_INPUT | GPIO_FLOAT | pin);
  return OK;
}
...
int bl702_config_uart_sel(gpio_pinset_t pinset, uint8_t sig_sel)
{
  irqstate_t flags;
  ...
  reg = getreg32(BL702_UART_SIG_SEL_0);
  reg &= ~(0xf << (sel_idx * 4));
  reg |= sig_sel << (sel_idx * 4);
  putreg32(reg, BL702_UART_SIG_SEL_0);
...
void bl702_gpiowrite(gpio_pinset_t pinset, bool value)
{
  ...
  if (value)
    {
      modifyreg32(BL702_GPIO_CFGCTL32, 0, (1 << pin));
    }
  else
    {
      modifyreg32(BL702_GPIO_CFGCTL32, (1 << pin), 0);
    }
}
...
{
  uint8_t pin = (pinset & GPIO_PIN_MASK) >> GPIO_PIN_SHIFT;
  return ((getreg32(BL702_GPIO_CFGCTL30) & (1 << pin)) ? 1 : 0);
}
