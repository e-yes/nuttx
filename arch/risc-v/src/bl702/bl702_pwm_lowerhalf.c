/****************************************************************************
 * arch/risc-v/src/bl702/bl702_pwm_lowerhalf.c
 *
...
#include "bl702_pwm_lowerhalf.h"
#include "hardware/bl702_pwm.h"
...
#    error "The maximum number of BL702 PWM channels is 5!"
#  endif
#endif

#ifdef CONFIG_BL702_PWM0_BUS_BCLK_SRC
...
static int bl702_pwm_setup(struct pwm_lowerhalf_s *dev);
static int bl702_pwm_shutdown(struct pwm_lowerhalf_s *dev);
static int bl702_pwm_start(struct pwm_lowerhalf_s *dev,
                           const struct pwm_info_s *info);
static int bl702_pwm_stop(struct pwm_lowerhalf_s *dev);
static int bl702_pwm_ioctl(struct pwm_lowerhalf_s *dev,
...
static const struct pwm_ops_s g_bl702_pwmops =
{
  .setup       = bl702_pwm_setup,
  .shutdown    = bl702_pwm_shutdown,
  .start       = bl702_pwm_start,
  .stop        = bl702_pwm_stop,
  .ioctl       = bl702_pwm_ioctl,
};

struct bl702_pwm_s g_bl702_pwm0 =
{
  .ops         = &g_bl702_pwmops,
...
{
  modifyreg32(BL702_PWM_N_CONFIG(ch), 0, CONFIG_PWM_STOP_EN);
  modifyreg32(BL702_PWM_N_CONFIG(ch), INTERRUPT_PWM_INT_ENABLE, 0);
}

static void pwm_channel_enable(uint8_t ch)
{
  modifyreg32(BL702_PWM_N_CONFIG(ch), CONFIG_PWM_STOP_EN, 0);
}

static void pwm_channel_init(uint8_t ch, struct bl702_pwm_ch_cfgs *ch_cfg)
{
  ...
  while (!(getreg32(BL702_PWM_N_CONFIG(ch)) & CONFIG_PWM_STS_TOP))
    {
      timeout_cnt--;
      if (timeout_cnt == 0)
        {
  ...
  modifyreg32(BL702_PWM_N_CONFIG(ch),
  ...
  putreg32(ch_cfg->clk_div, BL702_PWM_N_CLKDIV(ch));

  /* Config pwm period and duty */

  putreg32(ch_cfg->threshold1, BL702_PWM_N_THRE1(ch));
  putreg32(ch_cfg->threshold2, BL702_PWM_N_THRE2(ch));
  putreg32(ch_cfg->period, BL702_PWM_N_PERIOD(ch));

  /* Config interrupt pulse count */

  modifyreg32(BL702_PWM_N_INTERRUPT(ch), 0,
    (ch_cfg->int_pulse_cnt & PERIOD_PWM_PERIOD_MASK));
}

static int32_t pwm_init(uint8_t id, uint32_t freq)
{
  struct bl702_pwm_ch_cfgs pwm_cfg =
    {
      .ch = BL702_PWM_CH0,
#ifdef CONFIG_BL702_PWM0_BUS_BCLK_SRC
      .clk = BL702_PWM_CLK_BCLK,
#else
      .clk = BL702_PWM_CLK_XCLK,
#endif
      .stop_mode = BL702_PWM_STOP_ABRUPT,
      .pol = BL702_PWM_POL_NORMAL,
      .clk_div = 0,
      .period = 100,
      .threshold1 = 0,
      .threshold2 = 0,
...
static int bl702_pwm_duty(struct bl702_pwm_s *priv, uint8_t chan,
                          ub16_t duty)
{
  uint16_t period;
  uint16_t threshold1;
  ...
  tmp_val = getreg32(BL702_PWM_N_THRE1(chan));
  threshold1 = tmp_val & THRE1_PWM_THRE1_MASK;

  tmp_val = getreg32(BL702_PWM_N_THRE2(chan));
  threshold2 = tmp_val & THRE2_PWM_THRE2_MASK;

  tmp_val = getreg32(BL702_PWM_N_PERIOD(chan));
  period = tmp_val & PERIOD_PWM_PERIOD_MASK;

  threshold1 = 0;
  threshold2 = (uint16_t)((uint32_t)period * duty / 65535);

  putreg32(threshold1, BL702_PWM_N_THRE1(chan));
  putreg32(threshold2, BL702_PWM_N_THRE2(chan));
...
static int bl702_pwm_freq(struct bl702_pwm_s *priv, uint8_t chan,
                          uint32_t freq)
{
  ...
  putreg32(threshold1, BL702_PWM_N_THRE1(chan));
  putreg32(threshold2, BL702_PWM_N_THRE2(chan));
  putreg32(period, BL702_PWM_N_PERIOD(chan));
...
static int bl702_pwm_setup(struct pwm_lowerhalf_s *dev)
{
  int i;
  int ret = OK;
  struct bl702_pwm_s *priv = (struct bl702_pwm_s *)dev;
  ...
  for (i = 0; i < CONFIG_PWM_NCHANNELS; i++)
    {
      bl702_configgpio(priv->chan_pin[i]);
      pwm_init(i, BL_PWM_FREQ_DEFAULT);
    }
#else
  bl702_configgpio(priv->chan_pin[0]);
  ...
  for (i = 0; i < CONFIG_PWM_NCHANNELS; i++)
    {
      ...
      bl702_pwm_freq(priv, chan, info->frequency);
      bl702_pwm_duty(priv, chan, info->channels[i].duty);
      pwm_channel_enable(chan);
    }
#else
  bl702_pwm_freq(priv, 0, info->frequency);
  bl702_pwm_duty(priv, 0, info->duty);
