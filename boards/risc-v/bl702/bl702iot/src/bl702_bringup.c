/****************************************************************************
 * boards/risc-v/bl702/bl702iot/src/bl702_bringup.c
 *
...
#include <bl702_oneshot_lowerhalf.h>
...
#if defined(CONFIG_BL702_SPIFLASH)
#include <bl702_spiflash.h>
#endif

#if defined(CONFIG_BL702_BLE_CONTROLLER)
...
#endif
#endif /* CONFIG_BL702_BLE_CONTROLLER */

#ifdef CONFIG_FS_ROMFS
#include <nuttx/drivers/ramdisk.h>

#define BL702_XIP_START_ADDR    (0x23000000)
#define BL702_XIP_OFFSET        (*(volatile uint32_t *)0x4000B434)
#define BL702_ROMFS_FLASH_ADDR  (0x1C0000)
#define BL702_ROMFS_XIP_ADDR    (BL702_XIP_START_ADDR \
                                 + BL702_ROMFS_FLASH_ADDR \
                                 - BL702_XIP_OFFSET)
...
#if defined(CONFIG_BL702_WIRELESS)
extern int bl702_net_initialize(void);
#endif
...
extern void bl702_hci_uart_api_init(void *ble_uart_read,
                                    void *ble_uart_write);
...
int bthci_register(void)
{
  ...
  #elif defined(BL702_BLE_CONTROLLER)
...
void bl702_hci_uart_init(uint8_t uartid)
{
  int ret;
  ...
  bl702_hci_uart_api_init(ble_uart_read, ble_uart_write);
...
int bl702_bringup(void)
{
#if defined(CONFIG_TIMER) && defined(CONFIG_ONESHOT) && \
  defined(CONFIG_BL702_TIMER1)
...
#if defined(CONFIG_BL702_TIMER0)
  ret = bl702_timer_initialize("/dev/timer0", 0);
  if (ret < 0)
    {
      syslog(LOG_DEBUG,
...
#if defined(CONFIG_BL702_TIMER1) && !defined(CONFIG_ONESHOT)
  ret = bl702_timer_initialize("/dev/timer1", 1);
  if (ret < 0)
    {
      syslog(LOG_DEBUG,
...
#elif defined(CONFIG_BL702_TIMER1) && defined(CONFIG_ONESHOT)
  ...
  pwm = bl702_pwminitialize(0);
  if (pwm == NULL)
    {
      syslog(LOG_DEBUG, "ERROR: bl702_pwminitialize failed\n");
    }
  else
    {
  ...
  ret = bl702_wdt_initialize(CONFIG_WATCHDOG_DEVPATH);
  if (ret < 0)
    {
      syslog(LOG_DEBUG, "ERROR: bl702_wdt_initialize failed: %d\n", ret);
    }
#endif

#ifdef CONFIG_DEV_GPIO
  ret = bl702_gpio_initialize();
  if (ret < 0)
    {
...
#ifdef CONFIG_I2C
  i2c_bus = bl702_i2cbus_initialize(0);
  i2c_register(i2c_bus, 0);
#endif

#ifdef CONFIG_SPI
  spi_bus = bl702_spibus_initialize(0);
  spi_register(spi_bus, 0);
#endif

#ifdef CONFIG_BL702_SPIFLASH
  mtd_part = bl702_spiflash_alloc_mtdpart();
...
#endif /* CONFIG_BL702_SPIFLASH */

#ifdef CONFIG_BL702_WIRELESS
  bl702_set_em_sel(BL702_GLB_EM_8KB);

  bl702_net_initialize();
#endif

#ifdef CONFIG_RTC_DRIVER
  /* Instantiate the BL702 lower-half RTC driver */
...
#if defined(CONFIG_BL702_BLE_CONTROLLER)
  bl702_hci_uart_init(0);
#endif /* CONFIG_BL702_BLE_CONTROLLER */

#ifdef CONFIG_FS_ROMFS
  /* Create a ROM disk for the /sbin filesystem */

  ret = romdisk_register(0, BL702_ROMFS_XIP_ADDR,
                         512,
                         512);
  if (ret < 0)
    {
